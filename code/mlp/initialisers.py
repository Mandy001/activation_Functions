...
class SELUInit(object):
    ...
    def __call__(self, shape):
        ...
        # gaussian  obtain some weight values

        std = self.gain * (1. / shape[1])**0.5
        gaussianWeight = self.rng.normal(loc=0., scale=std, size=shape)

        alpha = 1.6733
        lamda = 1.0507
        gaussianWeightSELU = np.copy(gaussianWeight)
        gaussianWeightSELU[gaussianWeight<=0] = (np.exp(gaussianWeight[gaussianWeight<=0])-1) * alpha
        gaussianWeightSELU = gaussianWeightSELU * lamda

        return gaussianWeightSELU
...
class GlorotUniformInitFanIn(object):
    """Glorot and Bengio (2010) random uniform weights initialiser.
GlorotUniformInitFanIn
    References:
      [1]: Understanding the difficulty of training deep feedforward neural
           networks, Glorot and Bengio (2010)
    """
    ...
    def __call__(self, shape):
        ...
        print(str(shape[0])+':GlorotUniformInitFanIn ') # input
        std = self.gain * (1. / (shape[1]))**0.5
        half_width = 3.**0.5 * std
        return self.rng.uniform(low=-half_width, high=half_width, size=shape)


class GlorotUniformInitFanOut(object):
    """Glorot and Bengio (2010) random uniform weights initialiser.
GlorotUniformInitFanIn
    References:
      [1]: Understanding the difficulty of training deep feedforward neural
    ...
    def __call__(self, shape):
        ...
        print(str(shape[0])+':GlorotUniformInitFanOut') # output
