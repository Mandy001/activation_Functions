...
class LeakyReluLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        """
        self.alpha = 0.01
        outputs = np.maximum(inputs, inputs * self.alpha)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        gradients = np.ones(outputs.shape)
        gradients[outputs<0] = self.alpha
        gradients = gradients*grads_wrt_outputs
        return gradients
...
class ELULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        outputs[inputs<=0] = (np.exp(inputs[inputs<=0]) - 1) * self.alpha
        return outputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        # gradients = inputs #remove and replace with your code
        gradients =  np.ones(outputs.shape)
        # gradients[outputs<=0] = (np.exp(outputs[outputs<=0])) * self.alpha
        gradients[outputs<=0] = outputs[outputs<=0] + self.alpha
        gradients = gradients * grads_wrt_outputs
        # print(outputs)
        # print(gradients)
...
class SELULayer(Layer):
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        # gradients = inputs #remove and replace with your code
        #
        gradients = np.ones(outputs.shape) * self.lamda
        # gradients[outputs<=0] =  (np.exp(outputs[outputs<=0])) * self.alpha
        gradients[outputs<=0] = outputs[outputs<=0] + self.alpha * self.lamda
        gradients = gradients * grads_wrt_outputs
        return gradients
